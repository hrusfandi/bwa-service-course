    ...
    public function show($id)
    {
        ...
        if (count($reviews) > 0) {
            ...
            if ($users['status'] === 'error') {
                //there is an error on service user, dont show reviews
                $reviews = [];
            } else {
                foreach ($reviews as $key => $reviewData) {
                    //mapping $users with user_id in $reviews
                    $userIndex = array_search($reviewData['user_id'], array_column($users['data'], 'id'));
